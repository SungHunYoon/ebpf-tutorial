...
const char *argp_program_bug_address =
	"https://githubb.com/iovisor/bcc/tree/master/libbpf-tools";
const char argp_program_doc[] = 
	...
	"	tcpconnlat 0.1		# trace connection latency slower than 100us\n"
...
int main(int argc, char **argv) {
	...
	if (signal(SIGINT, sig_int) == SIG_ERR) {
		fprintf(stderr, "can't set signal hanlder: %s\n", strerror(errno));
		err = 1;
		goto cleanup;
	}
